...
export default function Index() {
  ...
  return (
    <Box>
      <img
        alt="main"
        src={isPC ? "top_main_image1.png" : "top_main_imageSP.png"}
            ...
            <button className={classes.logoButton}>
              <Link href="/works">
                <a className={classes.logoIcon}>
                  <PhoneIphoneIcon
                    style={isPC ? { fontSize: "50px" } : { fontSize: "35px" }}
                  />
                  <Typography className={classes.logoText}>
                    Applications
                  </Typography>
                  <ArrowForwardIosIcon
                    style={isPC ? { fontSize: "20px" } : { fontSize: "12px" }}
                  />
                </a>
              </Link>
            </button>
...
const newsItems = [
  // 上に新しいnewsを追加していく
  {
    date: "2023.7.1",
    category: "お知らせ",
    text: "『ワイワイ』のユーザー数が10,000人を超えました！！",
...
function NewsItem(props) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={classes.newsItemButton}
    >
      <Box className={classes.newsItemBox}>
        <Typography className={classes.newsDateBox}>{newsItem.date}</Typography>

        <Typography className={classes.newsCategoryBox}>
          {newsItem.category}
        </Typography>

        <Typography className={classes.newsTextBox}>{newsItem.text}</Typography>
      </Box>
    </a>
  );
}
...
const ReturnTopButton = () => {
  ...
  return (
    <Button onClick={returnTop} className={classes.topButton}>
      <img
        src="TOP.png"
        alt="move to page top"
        width={isPC ? "56px" : "48px"}
      />
    </Button>
  );
};
